...
# 初始化 FinMind API
api = DataLoader()
api.login_by_token(api_token='eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJkYXRlIjoiMjAyNS0wNy0yMyAwOTo0NzoyNyIsInVzZXJfaWQiOiJtOTYwNjI4NiIsImlwIjoiNjAuMjUwLjg5LjkxIn0.XlB7BmkZTaerzA_Wm4PB_MzfbL7i8-nqoQjj2UUV3KM')  # 預設放程式內，業務不用懂 token
...
if strategy_select == '均線交叉':
    short_ma = st.number_input("短天數:", value=5, min_value=1)
    long_ma = st.number_input("長天數:", value=20, min_value=1)
    param_values = {"short_ma": short_ma, "long_ma": long_ma}
elif strategy_select == 'KD交叉':
    kd_period = st.number_input("週期:", value=9, min_value=1)
    kd_threshold = st.number_input("閾值:", value=2)
    param_values = {"kd_period": kd_period, "kd_threshold": kd_threshold}
elif strategy_select == 'MACD交叉':
    fast = st.number_input("短期EMA:", value=12, min_value=1)
    slow = st.number_input("長期EMA:", value=26, min_value=1)
    signal = st.number_input("訊號EMA:", value=9, min_value=1)
    param_values = {"fast": fast, "slow": slow, "signal": signal}
elif strategy_select == 'RSI超買超賣':
    period = st.number_input("週期:", value=14, min_value=1)
    param_values = {"period": period}
...
if st.button("執行回測"):
    with st.spinner("回測進行中..."):
        ...
        # 技術指標
        if strategy_select == '均線交叉':
            df['短期均價'] = df['close'].rolling(param_values['short_ma'], min_periods=1).mean()
            df['長期均價'] = df['close'].rolling(param_values['long_ma'], min_periods=1).mean()
        elif strategy_select == 'KD交叉':
            low_min = df['min'].rolling(param_values['kd_period'], min_periods=1).min()
            high_max = df['max'].rolling(param_values['kd_period'], min_periods=1).max()
            ...
            df['EMA_fast'] = df['close'].ewm(span=param_values['fast'], adjust=False).mean()
            df['EMA_slow'] = df['close'].ewm(span=param_values['slow'], adjust=False).mean()
            df['DIF'] = df['EMA_fast'] - df['EMA_slow']
            df['DEA'] = df['DIF'].ewm(span=param_values['signal'], adjust=False).mean()
        elif strategy_select == 'RSI超買超賣':
            delta = df['close'].diff()
            gain = delta.where(delta>0,0)
            loss = -delta.where(delta<0,0)
            avg_gain = gain.ewm(alpha=1/param_values['period'], adjust=False).mean()
            avg_loss = loss.ewm(alpha=1/param_values['period'], adjust=False).mean()
        ...
        if strategy_select=='均線交叉':
            ...
            buy_kd = (diff>=param_values['kd_threshold']) & (diff_prev<param_values['kd_threshold'])
            sell_kd = (diff<=-param_values['kd_threshold']) & (diff_prev>-param_values['kd_threshold'])
        ...
        # 設定索引從1開始，欄位名稱叫「序號」
        trade_records.index = range(1, len(trade_records)+1)
        trade_records.index.name = "序號"
        ...
        st.markdown(
            ...
            f"**Buy & Hold累計報酬率:** <span style='color:{'red' if buy_hold_return>0 else 'green'}'>{buy_hold_return*100:.1f}%</span>",
